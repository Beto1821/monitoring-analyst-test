...
if 'last_simulation_type' not in st.session_state:
    st.session_state.last_simulation_type = None

# Executar simulação quando botão for clicado
if run_simulation:
    st.session_state.last_simulation_type = simulation_type
    ...
    with st.spinner(f"🎯 Executando {simulation_type}..."):
        try:
            if simulation_type == "🛒 Simulação de Checkouts":
                ...
                arrival_rate = st.sidebar.slider("Taxa Chegada (clientes/h)", 5, 50, 20, key="arrival")
